...
import { loginDestination } from '@/utils/platform';
...
const routes: Array<RouteRecordRaw> = [ // 声明路由表数组，类型约束为 RouteRecordRaw。
  ...
  },
  {
    ...
    redirect: '/workbench', // 默认进入 SGUI 用例工作台。
    children: [
      { path: 'workbench', name: 'GuiCaseWorkbench', component: () => import('@/views/GuiCaseWorkbenchView.vue'), meta: { requiresAuth: true } },
      {
...
router.beforeEach((to, _from, next) => { // 注册全局前置守卫，在每次导航确认前执行鉴权流程。
  ...
  if (!isLoggedIn && !isPublicRoute) { // 未登录且访问受保护路由时触发重定向。
    ...
    next(loginDestination(to.query.redirect)); // 已登录时回到工作台或原目标页面。
  } else {
