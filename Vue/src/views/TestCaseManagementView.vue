...
2. 按步骤真实操作网页并验证每项预期。不可用“输入框可见”代替“实际输入/选择成功”，不可只凭页面结构推断交互成功。
3. 每完成一个步骤，立即截图并上传到该用例，step_number 必须与原步骤一致，描述包含操作、实际观测和断言结果。使用本次执行独有的文件名，不复用历史截图。
4. 上传时使用截图工具返回的相对文件名或完整路径，平台会转换共享目录路径。上传失败最多纠正一次，不要猜测系统目录、调用无关技能或读取其他文件。
5. 最终逐步列出：操作、预期、实际、通过/失败/阻塞、截图。只要存在未执行、断言不符、截图未保存的步骤，就不能宣布整条用例通过。
6. 页面加载成功仅表示可访问。遇到验证码或访问拦截时记录并停止；不要绕过，也不要执行用例范围外的操作。
