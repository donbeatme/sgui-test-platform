...
/** Human-readable labels for the platform's compatible built-in tools. */
export function platformSkillLabel(name: string): string {
  return name === 'whart-test' ? 'SGUI Test Management' : name
}

export function platformSkillDescription(name: string, description: string): string {
  return isInternalPlatformSkill(name) ? description.replace(/WHartTest/g, 'SGUI') : description
}
