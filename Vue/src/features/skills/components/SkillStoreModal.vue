...
import { isInternalPlatformSkill, platformSkillLabel, platformSkillDescription } from '../utils/internalSkills'
...
function skillDisplayName(item: ManifestSkill): string {
  return platformSkillLabel(isEnglish.value && item.name_en ? item.name_en : item.name)
}

function skillDisplayDesc(item: ManifestSkill): string {
  return platformSkillDescription(item.name, isEnglish.value && item.description_en ? item.description_en : item.description)
}
