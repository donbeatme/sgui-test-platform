...
export const useApiTabsStore = defineStore('apiTabs', () => {
  ...
  function saveToLocalStorage(projectId?: number | null) {
    if (!projectId) return;
    try {
      ...
      localStorage.setItem(`${STORAGE_KEY}:${projectId}`, JSON.stringify({ tabs: data, activeTabId: activeTabId.value }));
    } catch { /* ignore quota errors */ }
  }

  function loadFromLocalStorage(projectId?: number | null) {
    clearInterfaceTabs();
    if (!projectId) return;
    try {
